import os
import socket
import socketserver
import sys

S = (
    ,
)


def rotate(l, k):
    k %= len(l)
    return l[k:] + l[:k]


def cross(m, s):
    res = 0
    for i in range(4):
        if m[i] == 1:
            res ^= s[i]
        elif m[i] == 2:
            if s[i] < 128:
                res ^= s[i] << 1
            else:
                res ^= (s[i] << 1) ^ 0x11b
        else:
            res ^= s[i]
            if s[i] < 128:
                res ^= s[i] << 1
            else:
                res ^= (s[i] << 1) ^ 0x11b
    return res


def mix_column(state):
    cols = [[state[i][j] for i in range(4)] for j in range(4)]
    base_m = [2, 3, 1, 1]
    res = [[cross(rotate(base_m, -j), cols[i]) for j in range(4)] for i in range(4)]
    state = [[res[i][j] for i in range(4)] for j in range(4)]
    return state


def ark(state, round_key, r):
    for i in range(4):
        for j in range(4):
            state[i][j] ^= round_key[4 * i + j]
        state[i][i] ^= r ^ 0x33
    return state


def aes(message: bytes, key: bytes):
    state = [
        [c for c in message[:4]],
        [c for c in message[4:8]],
        [c for c in message[8:12]],
        [c for c in message[12:]]
    ]

    state = [rotate(state[i], i) for i in range(4)]
    state = mix_column(state)

    state = ark(state, key, 0)
    for r in range(16):
        state = [[S[c] for c in row] for row in state]
        state = [rotate(state[i], i) for i in range(4)]
        state = mix_column(state)
        state = ark(state, rotate(key, r + 1), r + 1)
    return b''.join(bytes(row) for row in state)


def encrypt(message, key):
    if len(message) % 16 != 0:
        message = message + b'\x00' * (16 - len(message) % 16)
    cipher = b''
    for i in range(0, len(message), 16):
        cipher += aes(message[i:i + 16], key)
    return cipher


def run_challenge():
    try:
        sys.stdout.reconfigure(line_buffering=True)
    except AttributeError:
        pass


    key = os.urandom(16)
    cipher = encrypt(key, key)

    print(f"The whispers of secret: ", cipher.hex())

    MAX_QUERIES = 5000
    queries = 0

    while queries < MAX_QUERIES:
        try:
            message = input(f"Feed the BOX (hex message) [type 'end' to quit] ({queries}/{MAX_QUERIES}):")
        except EOFError:
            return

        message = message.strip()
        if message == 'end':
            print(f"\nLeaving the grid. Goodbye!")
            break

        try:
            m = bytes.fromhex(message)
            c = encrypt(m, key)
            print(f"[BOX Magic] Ciphertext morphed into: ", c.hex())
            print(f"\n")
            queries += 1
        except ValueError:
            print(f"Whoops! The BOX choked. Invalid hex string!\n")

    if queries >= MAX_QUERIES:
        print(f"[Alert] BOX exhausted! Query limit exceeded.")
        return

    try:
        user_key = input(f"The BOX demands the Master Key (in hex): ")
    except EOFError:
        return

    user_key = user_key.strip()
    try:
        user_key_bytes = bytes.fromhex(user_key)
        if user_key_bytes == key:
            try:
                flag = open('flag.txt', 'r').read().strip()
                print(f"\nBOX UNLOCKED! Here is your flag:")
                print(flag)
            except FileNotFoundError:
                print(f"\nBOX UNLOCKED! (flag file missing on server)")
        else:
            print(f"\n[ACCESS DENIED] The BOX rejects your key!\n")
    except ValueError:
        print(f"\n[ACCESS DENIED] That is not even valid hex!\n")


class ChallengeHandler(socketserver.BaseRequestHandler):

    def handle(self):
        sock = self.request
        try:
            sock.settimeout(300)  # 5 min idle cap per session
        except OSError:
            pass

        rfile = sock.makefile('r', encoding='utf-8', errors='replace', newline='\n')
        wfile = sock.makefile('w', encoding='utf-8', errors='replace', newline='\n', buffering=1)

        old_stdin, old_stdout = sys.stdin, sys.stdout
        sys.stdin, sys.stdout = rfile, wfile
        try:
            run_challenge()
        except (BrokenPipeError, ConnectionResetError, OSError, socket.timeout):
            pass
        finally:
            sys.stdin, sys.stdout = old_stdin, old_stdout
            try:
                wfile.flush()
            except OSError:
                pass
            try:
                sock.shutdown(socket.SHUT_RDWR)
            except OSError:
                pass


class ForkingTCPServer(socketserver.ForkingMixIn, socketserver.TCPServer):
    allow_reuse_address = True
    daemon_threads = True
    # Reap children eagerly so we don't accumulate zombies under CTF load
    max_children = 128


def serve(host, port):
    with ForkingTCPServer((host, port), ChallengeHandler) as srv:
        print(f"Listening on {host}:{port}", file=sys.stderr, flush=True)
        srv.serve_forever()


if __name__ == '__main__':
    # `python3 main.py serve [port]`  -> TCP server, forks per client
    # `python3 main.py`               -> single local session on stdin/stdout
    if len(sys.argv) >= 2 and sys.argv[1] == 'serve':
        port = int(sys.argv[2]) if len(sys.argv) >= 3 else 1337
        serve('0.0.0.0', port)
    else:
        run_challenge()
