...
function getPostTimestamp(post: CollectionEntry<'blog'>): number {
  const candidates = [
    path.join(BLOG_CONTENT_DIR, `${post.id}.mdx`),
    path.join(BLOG_CONTENT_DIR, `${post.id}.md`),
    path.join(BLOG_CONTENT_DIR, post.id, 'index.mdx'),
    path.join(BLOG_CONTENT_DIR, post.id, 'index.md'),
  ]
...
export async function getAllCategories(): Promise<Map<string, number>> {
  ...
  const categoryMap = new Map(EVENT_CATEGORIES.map((category) => [category, 0]))

  return posts.reduce((acc, post) => {
    acc.set(post.data.category, (acc.get(post.data.category) || 0) + 1)
    return acc
  }, categoryMap)
}
